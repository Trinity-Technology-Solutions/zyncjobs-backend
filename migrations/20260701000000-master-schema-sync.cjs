 ...
 * When you add a new model or column → update this file → commit → db:migrate
 */
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const CREATE_TABLES = {
      ...
      skills: async () => queryInterface.createTable('skills', {
      ...
      candidate_skills: async () => queryInterface.createTable('candidate_skills', {
        id:          { type: S.INTEGER, primaryKey: true, autoIncrement: true },
      ...
      talent_candidates: async () => queryInterface.createTable('talent_candidates', {
    ...
    for (const [table, createFn] of Object.entries(CREATE_TABLES)) {
      if (!existingTables.includes(table)) {
        console.log(`[SCHEMA SYNC] Creating missing table: ${table}`);
        await createFn();
      }
    }
    ...
    for (const [table, columns] of Object.entries(TABLE_COLUMNS)) {
      ...
      try {
        ...
        console.log(`[SCHEMA SYNC] Table "${table}" still missing — skipping columns`);
        continue;
      }
  ...
  async down() {
    // Intentionally empty — only additive changes, no destructive rollback
  },
};
