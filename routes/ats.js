...
async function getCompanyRecruiterEmails(companyId) {
  ...
  // Also include owner — use the real owner name from the User table (no hardcoded label)
  let ownerName = companyId;
  try {
    const owner = await User.findOne({
      where: { email: { [Op.iLike]: companyId } },
      attributes: ['name']
    });
    if (owner?.name) ownerName = owner.name;
  } catch { /* non-blocking */ }
  return [
    { email: companyId, name: ownerName, role: 'Owner' },
